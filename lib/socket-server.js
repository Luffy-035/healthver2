    ...
    io.on('connection', (socket) => {
      ...
      socket.on('join-chat', async (chatId) => {
        try {
          ...
          const hasAccess = (
            (socket.userRole === 'doctor' && chat.doctorId.userId === socket.userId) ||
            (socket.userRole === 'patient' && chat.patientId.userId === socket.userId)
          );
          ...
          socket.to(chatId).emit('user-joined', { userId: socket.userId, userRole: socket.userRole });
      ...
      socket.on('mark-read', async (data) => {
        try {
          ...
          socket.to(data.chatId).emit('messages-read', { readBy: socket.userId });
        } catch (error) {
